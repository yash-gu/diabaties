pipeline {
    ...
    stages {
        ...
        stage('Test container') {
            steps {
                sh '/Applications/Docker.app/Contents/Resources/bin/docker run -d --rm -p 8000:8000 --name diabetes-api-test diabetes-api'
                sh "sleep 5"
    ...
    post {
        always {
            sh '/Applications/Docker.app/Contents/Resources/bin/docker stop diabetes-api-test || true'
        }
    }
}
